...
#+author: Jeff Farr
#+property: header-args:emacs-lisp :tangle gptel-skills-roam.el
#+auto_tangle: y
...
2. **Metadata**: Extract from org properties (SKILL_DESCRIPTION, SKILL_INJECTION_MODE)
...
| =:SKILL_INJECTION_MODE:= | Where to inject (optional) | =system=, =context=, =user=, or =auto= |
...
2. Add required properties:
   ...
   - Add =SKILL_DESCRIPTION= and =SKILL_INJECTION_MODE=
...
Controls where skill content is injected. Values:

| Value | Injection Target | Use For |
|-------|-----------------|---------|
| =system= | =gptel--system-message= | Behavioral guidelines, general rules |
| =context= | =gptel-context= buffers | Reference docs, code examples |
| =user= | Top of prompt buffer | Immediate procedural steps |
| =auto= | Auto-detected | Let the system decide (default) |

Omit this property to default to =auto= mode.
...
The skill content is injected based on its injection mode.
#+end_example
