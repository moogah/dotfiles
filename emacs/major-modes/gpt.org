...
** Usage
Launch a dedicated LLM session with:
- =<SPC> l= - Opens a prompt to select from all configured backends and models
- Or run =M-x jf/gptel-launcher=

The launcher displays all available models in the format =Backend:model-name= (e.g.,
=ChatGPT:gpt-4o=, =Claude-thinking:claude-3-7-sonnet-20250219=, etc.). Each selection
creates or switches to a dedicated buffer named =*gptel-Backend-model*=, allowing
multiple concurrent sessions with different models.
