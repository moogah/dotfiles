...
*** Test Browse Function in Isolation

#+begin_src emacs-lisp :tangle no
;; Test the browse-sessions function structure
(defun jf/gptel-browse-sessions-test ()
...
#+RESULTS:
: jf/gptel-browse-sessions-test

*** Test Build Candidates Function

#+begin_src emacs-lisp :tangle no
;; Count parens in this function
(defun jf/gptel--build-session-candidates-test (session-dirs)
...
Returns alist of (display-string . (session-dir path leaf-node))."
  (let (candidates)                                           ; open 1
    (dolist (dir session-dirs)                                ; open 2
      (let ((metadata (jf/gptel--read-metadata dir)))         ; open 3
        (when metadata                                        ; open 4
          (let* ((tree (plist-get metadata :tree))            ; open 5
                 (leaves (jf/gptel--find-leaf-nodes tree))
                 (session-id (plist-get metadata :session_id)))
            ;; Add candidate for each leaf
            (dolist (leaf leaves)                             ; open 6
              (let* ((path (car leaf))                        ; open 7
                     (node (cdr leaf))
                     (depth (1- (length path)))
                ...
                (push (cons display (list :dir dir :path path :node node))
                      candidates)))))))                       ; close 7,6,5,4,3,2 = 6 closes needed!
    (nreverse candidates)))                                   ; close 1 = 1 close
#+end_src
