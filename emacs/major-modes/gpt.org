...
#+begin_src emacs-lisp
  (use-package gptel
    ...
    (gptel-cache t) ;; Enable prompt caching (for Anthropic models)
    :config
...
Tools are organized in the gpt-tools/ subdirectory for better maintainability.
Each category of tools is configured in its own file.
