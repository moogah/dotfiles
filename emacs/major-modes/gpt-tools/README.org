...
The subagent system enables LLM agents to delegate specialized tasks to isolated subagents
...
Built on [[https://github.com/karthink/gptel-agent][gptel-agent]] infrastructure for robust, battle-tested subagent execution.

** Architecture

*File-Based Agent Definitions*: Agents are defined in markdown/org files with YAML frontmatter

*Invisible Execution*: Subagents run in the background without creating visible buffers

*Standard Tool*: Use the =Agent= tool (from gptel-agent) to invoke subagents

*Complete Isolation*: Each subagent gets its own:
...
Subagents return only their final response (not full conversation history):
...
- Created visible buffers (=*gptel-subagent-*=)
- Hardcoded presets in elisp
- Custom =invoke_subagent= tool
...
- =subagent-tools.org= :: Legacy subagent implementation (superseded by gptel-agent integration)
