#+title: GPTEL Org-Roam Tools
#+author: Jeff Farr
#+property: header-args:emacs-lisp :tangle org-roam-tools.el
#+auto_tangle: y

* Introduction
This file defines gptel tools for interacting with org-roam knowledge bases.
These tools enable LLMs to search, read, create, and link org-roam nodes.
...
** Result Limiting
Prevent context overflow by limiting result size to 40,000 characters.
...
** Node Validation
Validate that node IDs exist before operations.
...
** Content Extraction
Functions to extract node content at different granularity levels.
Adapted from gptel-got patterns.
...
** search_roam_nodes
Search org-roam nodes by title, tags, or content.
...
** list_roam_nodes
List all org-roam nodes with optional filtering and sorting.
...
** read_roam_node
Read org-roam node content with configurable granularity.
...
Get detailed metadata for an org-roam node without reading content.
...
Find all nodes that link to a target node.
...
** create_roam_node
Create a new org-roam node programmatically.
...
Create a reference node that captures and summarizes external source material.

Reference nodes are stored in =~/org/roam/reference/= and use the =:ROAM_REFS:=
property to link to their source URL. These nodes serve as concise summaries of
external resources (web pages, papers, documentation) that are cited in your
knowledge base.
...
** link_roam_nodes
Create a bidirectional link between two org-roam nodes.
...
** add_roam_tags
Add tags to an existing org-roam node.
