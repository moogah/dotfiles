#+title: GPTEL Subagent Tools
#+author: Jeff Farr
#+property: header-args:emacs-lisp :tangle subagent-tools.el
#+auto_tangle: y

* Introduction

Tools for subagent delegation within gptel sessions. This enables primary agents
to delegate specialized tasks to isolated subagents (Explore and Plan types) that
run with their own tool sets, models, and configurations.
