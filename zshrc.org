...
Oh-My-Zsh is required as a dependency for many of the plugins we use through Antidote.
If not already installed, install it with:

#+begin_src sh :tangle no
# Install Oh-My-Zsh (run this manually if not installed)
sh -c "$(curl -fsSL https://raw.githubusercontent.com/ohmyzsh/ohmyzsh/master/tools/install.sh)"
#+end_src

Note: The installation script is set to =:tangle no= so it doesn't execute on every shell startup.
Run this manually once to install oh-my-zsh to =~/.oh-my-zsh=.

** Antidote Installation

Antidote is a high-performance ZSH plugin manager. Install it with Homebrew:

#+begin_src sh :tangle no
  # Install Antidote (run this manually if not installed)
